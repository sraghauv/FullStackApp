...
export default function PhotosUploader({addedPhotos, onChange}) {
    ...
    function selectFavorite(ev, filename) {
        ev.preventDefault();  
    ...
    return (
        ...
        <>
            <div className="flex gap-2">
                ...
                <button onClick={addPhotoByLink} className="rounded-2xl bg-gray-200 px-4">Add&nbsp;Photo</button>
            </div>
            <div className="gap-2 mt-4 grid grid-cols-3 lg:grid-cols-6 md:grid-cols-4">
                {addedPhotos.length > 0 && addedPhotos.map(link => (
                    <div className="h-32 flex relative" key={link}>
                        ...
                        <button  onClick={ev => selectFavorite(ev, link)} className="cursor-pointer absolute bottom-1  left-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3">
                            {link !== addedPhotos[0] && (
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
